...
mkdir -p ${out_dir}/{orig_space,mni_space}/tractograms/{orig,filtered,segmented,final}
...
do
	...
	# TODO: replace by the ensemble tractography strategy of Nasrin and trigeminal_first_order.sh
        ...
        scil_tractogram_apply_transform \
            ...
            ${mni_tracking_dir}/orig__${atlas_component/.nii.gz/_npv_${npv}.trk} \
...
do
    for npv in ${npv_list}
    do
        scil_tractogram_filter_by_roi ${mni_tracking_dir}/filtered_${nside}_mesencephalic_npv_${npv}.trk \
				      ${mni_tracking_dir}/segmented_${nside}_mesencephalic_npv_${npv}.trk  \
        ...
        scil_bundle_reject_outliers \
            ${mni_tracking_dir}/segmented_${nside}_mesencephalic_npv_${npv}.trk \
            ${mni_tracking_dir}/final_${nside}_mesencephalic_npv_${npv}.trk -f
    done
done
...
do
    for npv in ${npv_list}
    do
        scil_tractogram_filter_by_roi ${mni_tracking_dir}/filtered_${nside}_remaining_cp_npv_${npv}.trk \
				      ${mni_tracking_dir}/segmented_${nside}_remaining_cp_npv_${npv}.trk  \
        ...
        scil_bundle_reject_outliers \
            ${mni_tracking_dir}/segmented_${nside}_remaining_cp_npv_${npv}.trk \
            ${mni_tracking_dir}/final_${nside}_remaining_cp_npv_${npv}.trk  \
            --alpha 0.97
...
do
    for npv in ${npv_list}
    do
        scil_tractogram_filter_by_roi ${mni_tracking_dir}/filtered_${nside}_spinal_npv_${npv}.trk \
				      ${mni_tracking_dir}/segmented_${nside}_spinal_npv_${npv}.trk \
				      --drawn_roi ${mni_dir}/MNI/lowest_cut_brainstem.nii.gz 'any' 'include' \
				      --drawn_roi ${mni_dir}/MNI/coronal_plane.nii.gz 'any' 'include' \
				      -f
        scil_bundle_reject_outliers \
            ${mni_tracking_dir}/segmented_${nside}_spinal_npv_${npv}.trk \
            ${mni_tracking_dir}/final_${nside}_spinal_npv_${npv}.trk \
            -f
    done
done
echo "|------------- 5) Done -------------|"
