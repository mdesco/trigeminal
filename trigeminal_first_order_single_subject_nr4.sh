...
# If user provided -p/-e, run single combo; else run the 3x3 grid.
...
for nsub in ${subject_dir}/*/; do
    ...
    # --- DO NOT DELETE PREVIOUS RESULTS (removed rm -rf) ---
    ...
    for step_size in "${step_list[@]}"; do
      for theta in "${theta_list[@]}"; do
        ...
        # Per-combo directories so nothing gets overwritten
